...
* 当初始存储介质为 SSD，如果需要实现数据自动降冷，即数据是指从 SSD 介质迁移到 HDD 介质，则可以设置以下表属性（两者选其一），来指定数据自动降冷的时间点或者时间间隔。
...
* 目前 StarRocks 提供如下数据自动降冷的相关参数，对比如下：
  * `storage_cooldown_time`：表的属性，指定表中分区数据降冷时间间隔，由系统自动降冷表中到达时间点的分区数据，降低您的运维成本。并且表中数据按照分区粒度降冷，更加灵活。
  * `storage_cooldown_ttl`：表的属性，指定该表分区降冷时间点。
  * `storage_cooldown_second`：FE 静态参数，指定集群范围内表中数据自动降冷时间。
* 不设置该属性时，默认不进行自动降冷。
