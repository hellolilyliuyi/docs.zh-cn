...
[表达式分区](./expression_partitioning.md)，原称自动创建分区，最灵活易用，并且适用于大多数场景，比如按照连续日期范围或者枚举值来查询和管理数据。
...
StarRocks 会根据您创建分区时显式定义的分区范围与分区的映射关系将数据分配到相应的分区中。

创建分区的方式有三种：动态创建分区、批量创建分区和手动创建分区。
