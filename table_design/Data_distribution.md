...
- 建表时如何设置分桶数量
  ...
  - 方式一：自动设置分桶数量（推荐）
    ...
    - 哈希分桶表
      ...
      自 2.5.7 版本起，StarRocks 支持在建表时根据机器资源和数据量自动设置分区中分桶数量。
      > **注意**
      >
    ...
    - 随机分桶表
      ...
      自 2.5.7 版本起，StarRocks 支持在**建表时**根据机器资源和数据量自动设置分区中分桶数量。并且自 3.2 版本起，StarRocks 进一步优化了自动设置分桶数量的逻辑，除了支持在**建表时**自动设置分区中分桶数量，还支持**在导入过程中**根据集群能力和导入数据量等**按需动态增加**分区中分桶数量。在提高建表易用性的同时，还能提升大数据集的导入性能。
      > **说明**
      >
      > 单个分桶的大小默认为 `1024 * 1024 * 1024 B`（1 GB），在建表时您可以在 `PROPERTIES("bucket_size"="xxx")` 中指定单个分桶的大小，最大是支持为 4 GB。
...
- 新增分区时如何设置分桶数量
  ...
  - 方式一：自动设置分桶数量（推荐）
    ...
    - 哈希分桶表
      ...
      自 2.5.7 版本起， StarRocks 支持在新增分区时根据机器资源和数据量自动设置分区中分桶数量。
      > **注意**
      >
      > 如果表单个分区原始数据规模预计超过 100 GB，建议您手动设置分桶数量。
    ...
    - 随机分桶表
       ...
       > **说明**
       >
       > 单个分桶的大小默认为 `1024 * 1024 * 1024 B`（1 GB），在新增分区时您可以在 `PROPERTIES("bucket_size"="xxx")` 中指定单个分桶的大小，最大是支持为 4 GB。
