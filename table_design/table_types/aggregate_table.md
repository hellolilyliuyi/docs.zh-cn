...
- 从概念上，二者是不同的表达：聚合表是一种存储模型，是一种StarRocks表引擎类型，可在建表时指定AGGREGATE KEY生成；而单表物化视图则是一种预计算表达，其底层可以通过聚合表或者明细表来存储物化视图计算的结果，以用来透明加速而减少用户使用负担；
- 从表达上，聚合表创建表时需要手动指定列的聚合类型；而单表物化视图，优化器会自动地分析建MV的Query自动推导出聚合表达式，其支持的聚合函数类型同样受限于聚合表支持的聚合类型；同时单表物化视图，还可以表达数据加工后的再聚合（eg: sum(a + 10))；
- 从使用场景上，聚合表作为一个存储类型，需用户自定义存储表格式且直接查询该聚合表；而单表物化视图则自动地保障MV和基表的一致性，不需要用户干涉地创建，同时自动地改写用户基于基本的查询Query；
